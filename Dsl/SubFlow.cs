...
{
    /// <summary>
    /// A "stream of streams" sub-flow of data elements, e.g. produced by <see cref="Proto.Streams.Implementation.Fusing.GroupBy{T,TKey}"/>.
    ...
    public abstract class SubFlow<TOut, TMat, TClosed> : IFlow<TOut, TMat>
    {
        ...
        /// This is usually a bad idea when combined with <see cref="Proto.Streams.Implementation.Fusing.GroupBy{TIn,TKey}"/>
        /// since it can easily lead to deadlockâ€”the concatenation does not consume from the second
        /// substream until the first has finished and the <see cref="Proto.Streams.Implementation.Fusing.GroupBy{TIn,TKey}"/>
